...
def test_probe_gpu_detailed(
    ...
    all_corrects: list[torch.Tensor] = []
    ...
    with torch.no_grad():
        for i in range(0, len(labels), batch_size):
            ...
            preds_B = (logits_B > 0.0).long()
            correct_B = (preds_B == labels_B).long()

            all_logits.append(logits_B.detach().cpu())
            all_probs.append(probs_B.detach().cpu())
            all_preds.append(preds_B.detach().cpu())
            all_corrects.append(correct_B.detach().cpu())

    logits = torch.cat(all_logits) if all_logits else torch.empty(0, dtype=torch.float32)
    probs = torch.cat(all_probs) if all_probs else torch.empty(0, dtype=torch.float32)
    preds = torch.cat(all_preds) if all_preds else torch.empty(0, dtype=torch.long)
    corrects = (
        torch.cat(all_corrects) if all_corrects else torch.empty(0, dtype=torch.long)
    )

    accuracy = float(corrects.float().mean().item()) if corrects.numel() > 0 else 0.0
    return {
        "accuracy": accuracy,
