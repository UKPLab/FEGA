...
def _format_scr_run_label(column1_vals: tuple[str, str] | None) -> str:
    if column1_vals is None:
        return "scr"
    left = str(column1_vals[0]).replace("/", "_").replace(" ", "_")
    right = str(column1_vals[1]).replace("/", "_").replace(" ", "_")
    return f"scr_{left}_{right}"


def get_clean_prompt_score_payload(
    probes: dict[str, probe_training.Probe],
    chosen_classes: list[str],
    all_test_acts_BD: dict[str, torch.Tensor],
    probe_batch_size: int,
    dataset_name: str,
    ...
    per_class_summary: dict[str, dict[str, float | int | None]] = {}
    total_count = 0
    total_correct = 0
    total_score_sum = 0.0

    for class_name in chosen_classes:
        acts = all_test_acts_BD[class_name]
        labels = torch.full(
            (acts.shape[0],),
            dataset_info.POSITIVE_CLASS_LABEL,
            dtype=torch.long,
            device=acts.device,
        )
        detailed = probe_training.test_probe_gpu_detailed(
            acts,
            labels,
            probe_batch_size,
            probes[class_name],
        )
        logits = [float(v) for v in detailed["logits"]]  # type: ignore[arg-type]
        probabilities = [float(v) for v in detailed["probabilities"]]  # type: ignore[arg-type]
        predictions = [int(v) for v in detailed["predictions"]]  # type: ignore[arg-type]
        correct = [bool(v) for v in detailed["correct"]]  # type: ignore[arg-type]
        texts = (prompt_texts or {}).get(class_name, [])
        if texts and len(texts) != acts.shape[0]:
            raise ValueError(
                f"Prompt text count mismatch for class {class_name}: got {len(texts)} texts vs {acts.shape[0]} activations"
            )
            ...
            prompt_scores.append(
                {
                    ...
                    "entity_label": dataset_name,
                    "benchmark_run_label": benchmark_run_label,
                    "attribute_type": class_name,
                    "attribute_label": class_name,
                    "pair_role": "test_prompts",
                    "pair_index": idx,
                    "prompt": texts[idx] if idx < len(texts) else None,
                    "task_score": probabilities[idx],
                    "task_logit": logits[idx],
                    "predicted_label": predictions[idx],
                    "true_label": dataset_info.POSITIVE_CLASS_LABEL,
                    "is_correct": correct[idx],
                }
            )

        class_count = len(probabilities)
        class_correct = sum(1 for flag in correct if flag)
        per_class_summary[class_name] = {
            "count": class_count,
            "accuracy": (class_correct / class_count) if class_count else None,
            "mean_task_score": (
                sum(probabilities) / class_count if class_count else None
            ),
        }
        total_count += class_count
        total_correct += class_correct
        total_score_sum += float(sum(probabilities))

    return {
        "dataset_name": dataset_name,
        "benchmark_run_label": benchmark_run_label,
        "column1_vals": list(column1_vals) if column1_vals else None,
        "metric_label": "task_score",
        "metric_description": "Sigmoid probability assigned by the clean intended probe to the prompt's true class detector.",
        "prompt_scores": prompt_scores,
        "per_class_summary": per_class_summary,
        "overall_summary": {
            "count": total_count,
            "accuracy": (total_correct / total_count) if total_count else None,
            "mean_task_score": (total_score_sum / total_count) if total_count else None,
        },
    }
...
def get_dataset_activations(
    ...
    tokenized_train_data = dataset_utils.tokenize_data_dictionary(
        train_data,
        model.tokenizer,  # type: ignore
        config.context_length,
        device,
    )
    tokenized_test_data = dataset_utils.tokenize_data_dictionary(
        test_data,
        model.tokenizer,  # type: ignore
        config.context_length,
        device,
    ...
    if return_datasets:
        return train_data, test_data, all_train_acts_BLD, all_test_acts_BLD
...
def run_eval_single_dataset(
    ...
    test_data: dict[str, list[str]] | None = None

    if not os.path.exists(activations_path):
        if config.lower_vram_usage:
            model = model.to(device)  # type: ignore
        train_data, test_data, all_train_acts_BLD, all_test_acts_BLD = get_dataset_activations(
            dataset_name,
            config,
            model,
        ...
        loaded_test_data = acts.get("test_data")
        if isinstance(loaded_test_data, dict):
        ...
        if test_data is None:
            _train_data, rebuilt_test_data = dataset_creation.get_train_test_data(
                dataset_name,
                config.perform_scr,
                config.train_set_size,
                config.test_set_size,
            ...
            if not config.perform_scr:
                rebuilt_test_data = dataset_utils.filter_dataset(
                    rebuilt_test_data, chosen_classes
                )
            test_data = rebuilt_test_data
    ...
    mean_test_acts_BD = activation_collection.create_meaned_model_activations(
        all_test_acts_BLD
    )
    benchmark_run_label = (
        _format_scr_run_label(column1_vals) if config.perform_scr else "tpp"
    )
    instance_metric_payload = get_clean_prompt_score_payload(
        llm_probes,  # type: ignore[arg-type]
        chosen_classes,
        mean_test_acts_BD,
        config.probe_test_batch_size,
        dataset_name,
        benchmark_run_label,
        prompt_texts=test_data,
        column1_vals=column1_vals,
    )

    return ablated_class_accuracies, llm_test_accuracies, instance_metric_payload  # type: ignore
...
def run_eval_single_sae(
    ...
    artifacts_folder: str,
    save_activations: bool = True,
) -> tuple[dict[str, float | dict[str, float]], dict, dict[str, Any]]:
    ...
    instance_metric_results: dict[str, Any] = {}
    ...
    for dataset_name in config.dataset_names:
        if config.perform_scr:
            ...
            for column1_vals in column1_vals_list:
                ...
                raw_results, llm_clean_accs, instance_metric_payload = run_eval_single_dataset(
                    dataset_name,
                    config,
                    sae,
                    model,
                ...
                instance_metric_results[run_name] = instance_metric_payload
    ...
    return results_dict, per_dataset_class_results, instance_metric_results  # type: ignore
...
def run_eval(
    ...
    for sae_release, sae_object_or_id in tqdm(
        ...
        eval_result_unstructured = {
            "instance_metrics": instance_metric_results,
            "per_dataset_class_results": per_dataset_class_results,
        }
        if per_dataset_class_results:
            eval_result_unstructured.update(per_dataset_class_results)
        if eval_type == EVAL_TYPE_ID_SCR:
            eval_output = ScrEvalOutput(
                ...
                eval_result_unstructured=eval_result_unstructured,
            )
