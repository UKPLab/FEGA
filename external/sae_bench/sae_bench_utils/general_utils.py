...
def average_results_dictionaries(
    ...
    for dataset_name in dataset_names:
        ...
        for metric_name, metric_value in dataset_results.items():
            if isinstance(metric_value, list): # to handle isolation_instances and cause_instances which are list of dicts
                if metric_name not in aggregated_results:
                    aggregated_results[metric_name] = []

                aggregated_results[metric_name].extend(metric_value) # becomes a list of dict

            else:
                if metric_name not in aggregated_results:
                    aggregated_results[metric_name] = []

                aggregated_results[metric_name].append(metric_value) # becomes a list of float

    averaged_results = {}

    for metric_name, values in aggregated_results.items():
        if not isinstance(values[0], dict): # to handle isolation_instances and cause_instances
            average_value = sum(values) / len(values)
            averaged_results[metric_name] = average_value
        else:
            averaged_results[metric_name] = values
