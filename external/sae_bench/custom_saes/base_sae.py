...
def resolve_repo_file(
    ...
    cache_dir = Path(local_dir).expanduser()
    relative_path = Path(subfolder, filename) if subfolder else Path(filename)
    candidates = [cache_dir / relative_path]

    if cache_dir.name == repo_id.replace("/", "_"):
        candidates.append(cache_dir.parent / relative_path)
