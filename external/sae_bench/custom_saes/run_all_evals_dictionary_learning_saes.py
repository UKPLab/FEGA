...
def get_all_hf_repo_autoencoders(
    repo_id: str, download_location: str = "data/downloaded_saes"
) -> list[str]:
    config_dir = Path(download_location) / repo_id.replace("/", "_")
    config_locations = sorted(config_dir.rglob("config.json"))
    ...
    return [
        config.parent.relative_to(config_dir).as_posix() for config in config_locations
    ]
...
def load_dictionary_learning_sae(
    ...
    dtype: torch.dtype,
    layer: int | None = None,
    download_location: str = "data/downloaded_saes",
) -> base_sae.BaseSAE:
    download_location = os.path.join(download_location, repo_id.replace("/", "_"))

    config_file = base_sae.resolve_repo_file(
        repo_id=repo_id,
    ...
    for eval_type in tqdm(eval_types, desc="Evaluations"):
        ...
        if eval_type == "unlearning":
            if not os.path.exists(
                "./external/sae_bench/evals/unlearning/data/bio-forget-corpus.jsonl"
            ):
                print(
