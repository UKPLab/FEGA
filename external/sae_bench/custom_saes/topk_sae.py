...
def load_dictionary_learning_topk_sae(
    ...
    dtype: torch.dtype,
    layer: int | None = None,
    local_dir: str = "data/downloaded_saes",
    use_threshold_at_inference: bool = False,
) -> TopKSAE:
    assert "ae.pt" in filename

    path_to_params = base_sae.resolve_repo_file(
        repo_id=repo_id,
        filename=filename,
        local_dir=local_dir,
    )

    pt_params = torch.load(
        path_to_params, map_location=torch.device("cpu"), weights_only=True
    )

    config_filename = filename.replace("ae.pt", "config.json")
    path_to_config = base_sae.resolve_repo_file(
        repo_id=repo_id,
